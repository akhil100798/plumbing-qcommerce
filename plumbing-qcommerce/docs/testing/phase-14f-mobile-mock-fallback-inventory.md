...
- Step 3 commit target: `Phase 14F fix P1 mobile real API gaps`
...
- P1 findings blocked by missing backend endpoint: `0`
- Staging behavior after Step 3:
  - Customer nearby-plumber edge flows fail closed when `EXPO_PUBLIC_EDGE_URL` is blank
  - Customer splash/profile/booking flows no longer create fake identity or fake successful assignment
  - Plumber websocket/profile/wallet/earnings/material-tracking flows now show explicit unavailable or pending state instead of fake success
  - Store wallet/notifications/analytics/dispatch/material-request/profile/dashboard/login flows now use real backend data or explicit unavailable states
- Validation completed:
  - `customer-app`: `npm run typecheck`, `npm test`, `npm run build` — PASS
  - `plumber-app`: `npm run typecheck`, `npm test`, `npm run build` — PASS
  - `store-app`: `npm run typecheck`, `npm test`, `npm run build` — PASS
- Live Render smoke completed with:
  - `/health/live`: `UP`
  - `customer@plumbcommerce.com`: login PASS
  - `plumber@plumbcommerce.com`: login PASS
  - `store@plumbcommerce.com`: login PASS
- Remaining counts after P1 implementation:
  - `P0`: `0`
  - `P1`: `0`
  - `P2`: `10`
  - `P3`: `8`
- Mobile staging verdict after P1:
  - `customer-app`: `PARTIAL`
  - `plumber-app`: `PARTIAL`
  - `store-app`: `PARTIAL`
  - `mobile staging`: `PARTIAL`
