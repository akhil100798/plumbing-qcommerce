...
export function RegisterScreen({ navigation }: Props) {
  ...
  const register = async () => {
    ...
    if (Object.values(form).some((value) => !value.trim())) {
      setErrorMessage('Complete all manager, store, address, coordinate, and password fields.');
      return;
    }
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email.trim()) || !/^[6-9]\d{9}$/.test(form.phone)) {
      setErrorMessage('Enter a valid email address and 10-digit Indian mobile number.');
      return;
    }
...
const styles = StyleSheet.create({
  ...
  row: { flexDirection: 'row', gap: 10 }, half: { flex: 1 }, button: { marginTop: 8 }, feedback: { color: colors.error, backgroundColor: colors.errorContainer, borderRadius: 8, padding: 10, marginBottom: 12 },
