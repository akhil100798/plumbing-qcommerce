...
const OUTPUT_DIR = path.resolve(__dirname, '../../docs/evidence/phase-14h-live-mobile-uat/store-app');
...
(async () => {
  console.log('Starting store-app screenshot capture...');
  ...
  try {
    // 1. Navigate to store-app Expo Web
    ...
    await page.keyboard.type('5555555603');
    ...
    // View Inventory Tab
    const inventoryTab = page.getByText('Inventory', { exact: false }).first();
    if (await inventoryTab.count() > 0) {
      await inventoryTab.click();
      await page.waitForTimeout(2000);
      await page.screenshot({ path: path.join(OUTPUT_DIR, '06-inventory.png') });
      console.log('Saved 06-inventory.png');
    }

    // View Profile/Account Tab
